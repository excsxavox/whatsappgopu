...
La aplicación debe procesar cada tipo de nodo en un flujo de WhatsApp y mantener la conversación dentro del flujo, sin que el usuario se salga del flujo hasta completarlo o abandonarlo.
...
Una sesión es el estado actual de una conversación dentro de un flujo específico. Cada conversación (identificada por `conversationId`) debe tener una sesión activa que rastree:
...
**NOTA:** Este nodo es opcional. Si un nodo TEXT tiene `waitForResponse=true`, la respuesta se guarda automáticamente. El nodo RESPONSE permite validación adicional.
...
```
Nodo TEXT → Edge único → Siguiente nodo
```

**Nodo TEXT (waitForResponse=true):**
```
Nodo TEXT → Espera respuesta usuario → Nodo RESPONSE (opcional) → Edge → Siguiente nodo
```

**Nodo BUTTONS:**
```
Nodo BUTTONS → Espera respuesta usuario (botón presionado) → Nodo CONDITION → Edge "yes" o "no" → Siguiente nodo
```

**Nodo HTTP:**
```
Nodo HTTP → Ejecuta llamada → Edge único → Siguiente nodo (generalmente CONDITION)
```

**Nodo CONDITION:**
```
Nodo CONDITION → Evalúa condición → Edge "yes" o "no" → Siguiente nodo según resultado
```
...
**Opción A:** Guardar error en la variable de respuesta
